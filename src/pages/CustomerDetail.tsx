...
const CustomerDetail = () => {
  ...
  const calculatePenaltyInfo = (customer: Customer) => {
    if (!customer.deadlineDate) return null;
    ...
    if (customer.paymentCategory === 'daily') {
      // Daily: (interest amount / due days) × excess days
      penaltyPerPeriod = customer.interestAmount / customer.numberOfDays;
      periodLabel = 'day';
      excessPeriods = excessDays;
      calculationDetails = `₹${customer.interestAmount.toLocaleString()} ÷ ${customer.numberOfDays} days = ₹${penaltyPerPeriod.toLocaleString(undefined, { maximumFractionDigits: 2 })} per day`;
    } else if (customer.paymentCategory === 'weekly') {
      // Weekly: (interest amount / due weeks) × excess weeks
      const originalNumberOfWeeks = customer.numberOfWeeks || customer.numberOfDays; // Use original weeks
      penaltyPerPeriod = customer.interestAmount / originalNumberOfWeeks;
      periodLabel = 'week';
      excessPeriods = Math.floor(excessDays / 7);
      calculationDetails = `₹${customer.interestAmount.toLocaleString()} ÷ ${originalNumberOfWeeks} weeks = ₹${penaltyPerPeriod.toLocaleString(undefined, { maximumFractionDigits: 2 })} per week`;
    } else if (customer.paymentCategory === 'monthly') {
      // Monthly: (interest amount / due months) × excess months
      const originalNumberOfMonths = customer.numberOfMonths || customer.numberOfDays; // Use original months
      penaltyPerPeriod = customer.interestAmount / originalNumberOfMonths;
      periodLabel = 'month';
      excessPeriods = Math.floor(excessDays / 30);
      calculationDetails = `₹${customer.interestAmount.toLocaleString()} ÷ ${originalNumberOfMonths} months = ₹${penaltyPerPeriod.toLocaleString(undefined, { maximumFractionDigits: 2 })} per month`;
    }
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="p-3 rounded-lg bg-finance-red/5 border border-finance-red/20 space-y-2">
                    <p className="text-sm text-finance-red">
                      <strong>Rate Calculation:</strong> {penaltyInfo.calculationDetails}
                    </p>
                    <p className="text-sm text-finance-red">
                      <strong>Penalty Calculation:</strong> ₹{penaltyInfo.penaltyPerPeriod.toLocaleString(undefined, { maximumFractionDigits: 2 })} × {penaltyInfo.excessPeriods} {penaltyInfo.periodLabel}s = ₹{penaltyInfo.totalPenalty.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                    </p>
                  </div>
                </CardContent>
              </Card>
